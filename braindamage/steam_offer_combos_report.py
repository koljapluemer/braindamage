"""Static HTML report of braindamage.steam_offer_combos' results -- the best
mono trade-up combos actually buyable right now from fresh Steam Community
Market listings already on disk. Same "single self-contained file, no JS"
shape as braindamage.mono_offer_combos_report (its CSFloat counterpart), and
reuses the same escaping/formatting helpers so numbers render identically
across every report this app writes.
"""
...
from .steam_offer_combos import MAX_OFFER_AGE
...
def _offers_table(combo: ComboResult) -> str:
    ...
    for offer in sorted(combo.offers, key=lambda o: o.price):
        rows.append(
            ...
            f"<td class='num'>{_esc(offer.pattern_seed) if offer.pattern_seed is not None else '—'}</td>"
    ...
    return (
        ...
        "<th>Wear</th><th class='num'>Float</th><th class='num'>Pattern seed</th>"
        "<th class='num'>Price</th><th class='num'>Fetched</th>"
...
def render_report(combos: list[ComboResult], *, top_n: int) -> str:
    ...
    if not combos:
        cards = (
            ...
            f"&ge;10 fresh (&lt;{max_age_hours:.0f}h old) Steam Market listings on disk, "
            "or the extension hasn't fetched anything yet (see the Steam Market offer scraper "
...
<head>
<meta charset="utf-8">
<title>Steam Market mono trade-up buy combos</title>
<style>{_CSS}</style>
</head>
<body>
<div class="wrap">
<header class="report-header">
<h1>Steam Market mono trade-up buy combos</h1>
<p>Generated {generated_at} &nbsp;·&nbsp; Top {top_n} mono trade-up combo(s), by real expected value, buyable
right now from Steam Community Market listings already on disk and younger than {max_age_hours:.0f}h.</p>
...
<footer>Input prices are each listing's real Steam Community Market ask (scraped, USD only); output prices are
this app's regular price signals, net of Steam Community Market's real per-sale fee (5% Steam + 10% game fee,
computed cent-exact the way Steam itself computes it -- about {steam_fees.NOMINAL_CUT_OF_GROSS:.0%} of gross for
most prices, more for very cheap listings), same ones contract simulation uses.</footer>
</div>
</body>
</html>"""
