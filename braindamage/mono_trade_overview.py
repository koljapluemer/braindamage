...
def _latest_offer_prices_by_wear(skin_id: str) -> dict[str, float]:
    """Cheapest last-known listing per wear, with repeated sightings deduped."""
    by_wear: dict[str, dict[tuple[float | None, int | None, float], signals.SteamOfferSignal]] = defaultdict(dict)
    for offer in signals.read_steam_offers(skin_id):
        if offer.wear_name:
            key = (offer.float_value, offer.pattern_seed, offer.price)
            current = by_wear[offer.wear_name].get(key)
            if current is None or offer.fetched_at > current.fetched_at:
                by_wear[offer.wear_name][key] = offer

    return {wear: min(offer.price for offer in offers.values()) for wear, offers in by_wear.items()}


def _best_known_input_price(skin: Skin, legacy_prices: dict[str, tuple[float, datetime]]) -> float | None:
    """Lowest wear price, preferring scraped offers over legacy price signals."""
    offer_prices = _latest_offer_prices_by_wear(skin.id)
    candidates = []
...
def _best_naive_ev(
    ...
    for skin in skins:
        table = mono_trade_table.build_table(
            session, skin, legacy_prices_by_skin=legacy_prices_by_skin
        )
        values.extend(
            row["ev_cell"]["value"]
...
def build_overview(
    session: Session, *, rarities: list[str] | None = None, stattrak_values: list[bool] | None = None
) -> dict:
    ...
    for (_collection_id, rarity, stattrak), skins in groups.items():
        ...
        if ev is None:
            ...
            ev = _best_naive_ev(session, skins, legacy_prices_by_skin)

        prices = {skin.id: _best_known_input_price(skin, legacy_prices(skin.id)) for skin in skins}
